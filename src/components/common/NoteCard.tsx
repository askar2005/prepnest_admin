    ...
    <div className="rounded-xl border border-slate-200 bg-white shadow-soft hover:shadow-md transition-shadow">
      <div className="p-4 border-b border-slate-100">
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1 min-w-0"><h3 className="text-sm font-semibold text-slate-900 truncate">{note.title}</h3></div>
          <div className="flex flex-col gap-1 ml-2 shrink-0"><span className={`text-[10px] px-2 py-0.5 rounded font-medium ${publishedColor}`}>{note.isPublished ? 'Published' : 'Draft'}</span></div>
        </div>
