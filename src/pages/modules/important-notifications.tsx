  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2 sm:gap-3">
        ...
        <button onClick={() => setShowFilters(!showFilters)} className="sm:hidden flex items-center gap-1.5 px-3 py-2 rounded-xl border border-slate-200 text-sm text-slate-600 hover:bg-slate-50">
          <Filter className="w-4 h-4" /> Filters
        </button>
        <div className={`${showFilters ? 'flex' : 'hidden'} sm:flex flex-wrap gap-2 sm:gap-3 w-full sm:w-auto`}>
          <Select value={filterCat} onChange={(e) => setFilterCat(e.target.value)} className="w-full sm:w-44">
            <option value="">All Categories</option>
            {CATEGORIES.map(c => <option key={c} value={c}>{CATEGORY_LABELS[c] || c}</option>)}
          </Select>
          <Select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="w-full sm:w-36">
            <option value="">All Status</option>
            {STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
          </Select>
          <Select value={filterPriority} onChange={(e) => setFilterPriority(e.target.value)} className="w-full sm:w-36">
      ...
      <div className="rounded-xl border border-slate-200 bg-white overflow-hidden shadow-soft">
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-semibold text-slate-600 text-xs uppercase hidden sm:table-cell">Category</th>
                <th className="text-left px-4 py-3 font-semibold text-slate-600 text-xs uppercase hidden md:table-cell">Priority</th>
                <th className="text-center px-4 py-3 font-semibold text-slate-600 text-xs uppercase hidden md:table-cell">Status</th>
                <th className="text-center px-4 py-3 font-semibold text-slate-600 text-xs uppercase hidden lg:table-cell">Views</th>
                <th className="text-left px-4 py-3 font-semibold text-slate-600 text-xs uppercase hidden lg:table-cell">Published</th>
                ...
                <tr key={n.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      {n.thumbnailUrl ? (
                        <img src={previewUrl(n.thumbnailUrl)} alt="" className="w-10 h-10 rounded-lg object-cover bg-slate-100 shrink-0 hidden sm:block" />
                      ) : (
                        <div className="w-10 h-10 rounded-lg bg-slate-100 flex items-center justify-center shrink-0 hidden sm:flex">
                          <Megaphone className="w-4 h-4 text-slate-400" />
                        </div>
                      )}
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-slate-900 truncate max-w-[180px] sm:max-w-[250px]">{n.title}</p>
                        <div className="flex sm:hidden items-center gap-1 mt-0.5">
                          <span className="text-[10px] text-slate-500">{CATEGORY_LABELS[n.category] || n.category}</span>
                          {n.status && <><span className="text-slate-300">·</span><span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${STATUS_STYLES[n.status] || ''}`}>{n.status}</span></>}
                        </div>
                        {n.summary && <p className="text-xs text-slate-400 truncate max-w-[180px] sm:max-w-[250px] hidden sm:block">{n.summary}</p>}
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3 hidden sm:table-cell"><span className="text-xs text-slate-600">{CATEGORY_LABELS[n.category] || n.category}</span></td>
                  <td className="px-4 py-3 hidden md:table-cell"><span className={`inline-block px-2 py-0.5 text-[10px] font-semibold rounded-full ${PRIORITY_STYLES[n.priority] || ''}`}>{n.priority}</span></td>
                  <td className="px-4 py-3 text-center hidden md:table-cell"><span className={`inline-block px-2 py-0.5 text-[10px] font-semibold rounded-full ${STATUS_STYLES[n.status] || ''}`}>{n.status}</span></td>
                  <td className="px-4 py-3 text-center text-sm text-slate-600 hidden lg:table-cell">{n.views}</td>
                  <td className="px-4 py-3 text-xs text-slate-400 hidden lg:table-cell">{n.publishDate ? new Date(n.publishDate).toLocaleDateString() : '-'}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button onClick={() => handleDuplicate(n)} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors hidden sm:block" title="Duplicate"><Copy className="w-3.5 h-3.5" /></button>
                      {n.status !== 'PUBLISHED' && <button onClick={() => handlePublish(n.id)} className="p-1.5 rounded-lg hover:bg-green-50 text-slate-400 hover:text-green-600 transition-colors hidden sm:block" title="Publish"><Send className="w-3.5 h-3.5" /></button>}
                      {n.status !== 'ARCHIVED' && <button onClick={() => handleArchive(n.id)} className="p-1.5 rounded-lg hover:bg-orange-50 text-slate-400 hover:text-orange-600 transition-colors hidden sm:block" title="Archive"><Archive className="w-3.5 h-3.5" /></button>}
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 px-4 py-3 border-t border-slate-100 bg-slate-50/50">
      ...
      {/* Create/Edit Modal */}
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-4 sm:pt-10 pb-4 sm:pb-10 bg-black/40 backdrop-blur-sm overflow-y-auto" onClick={() => setShowModal(false)}>
          <div className="w-full max-w-3xl mx-2 sm:mx-4 bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden my-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-4 sm:p-6 border-b border-slate-100">
              <h2 className="text-lg font-bold text-slate-900">{editing ? 'Edit Notification' : 'Create Notification'}</h2>
            </div>
            <div className="p-4 sm:p-6 space-y-4 sm:space-y-5 max-h-[60vh] sm:max-h-[70vh] overflow-y-auto">
              <div className="grid sm:grid-cols-2 gap-4">
                <label className="space-y-1 sm:col-span-2"><span className="text-xs text-slate-500">Title *</span><Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Notification title" /></label>
                <label className="space-y-1 sm:col-span-2"><span className="text-xs text-slate-500">Short Summary</span><Input value={form.summary} onChange={(e) => setForm({ ...form, summary: e.target.value })} placeholder="Brief summary for cards" /></label>
                <label className="space-y-1 sm:col-span-2"><span className="text-xs text-slate-500">Description (Rich Text)</span><TextArea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={5} placeholder="Full notification content with HTML support" /></label>
                ...
                <label className="space-y-1 sm:col-span-2"><span className="text-xs text-slate-500">Banner URL</span><Input value={form.bannerUrl} onChange={(e) => setForm({ ...form, bannerUrl: e.target.value })} placeholder="https://..." /></label>
                <label className="space-y-1"><span className="text-xs text-slate-500">Attachment URL</span><Input value={form.attachmentUrl} onChange={(e) => setForm({ ...form, attachmentUrl: e.target.value })} placeholder="https://..." /></label>
            ...
            <div className="p-4 sm:p-6 border-t border-slate-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-3">
              <Button variant="secondary" onClick={() => setShowModal(false)} className="w-full sm:w-auto">Cancel</Button>
              <Button onClick={handleSave} disabled={saving} className="w-full sm:w-auto">{saving ? 'Saving...' : editing ? 'Update' : 'Create'}</Button>
            </div>
          </div>
        </div>
      )}
